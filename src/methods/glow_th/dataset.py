...
class GlowTHDataset(Dataset):
  def __init__(self, list_samples, method, keep_rate = 0.3):
    super(GlowTHDataset, self).__init__()
    self.method = method
    self.list_samples = []
    self.keep_rate = keep_rate
    for (x, y) in list_samples:
      for i in range(len(x.vars)):
        if random.random() < keep_rate:
          self.list_samples.append((i, x, y))

  def __len__(self):
    return len(self.list_samples)

  def __getitem__(self, idx):
    (i, input, output) = self.list_samples[idx]
    x = self.method.preproc_input(i, input, output)
    y = self.method.preproc_output(i, input, output, 'onehot')
    y_ids = self.method.preproc_output(i, input, output, 'idx').to(y.device)
    return x, y_ids, y
